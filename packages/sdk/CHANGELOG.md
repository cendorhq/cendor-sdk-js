...
- b4609ca: Resuming an unfinished checkpoint whose transcript already ends with the final assistant answer no
  longer re-invokes the model.

  Every run path saves the answering turn with `done: false` _before_ the `done: true` save lands, so
  ...
  _model_ re-issues the same call is its own sampling decision. Make tools used under `checkpoint`
  idempotent.
